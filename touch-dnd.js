...
(function (factory) {
  ...
  function trigger(el, name, originalEvent, arg, wait) {
    if (!el[0]) return
  ...
  Droppable.prototype.drop = function (e) {
    ...
    if (!drop.isDefaultPrevented()) {
      if (handler) {
        ...
        // console.log(e)
        // console.log()
        if ($(this.el).children().length > 0 && !$(this.el).hasClass("droppable-basket")) {
          dragging.el.parent().append($(this.el).children())
        }
        ...
        // trigger(this.el, 'droppable:drop', e, evtObj)
        wordDropEvent(e, evtObj)
      }
    }
  }
